	...
	Copyright (c) 2004-2012 Jan Seiffert  ->  kaffeemonster@googlemail.com
...
----------------
	...
	* libcurl >= Version 7.19.4
		[note: From time to time there may be the need to connect to a
		       ...
		       For that reason the GWC-specification _demands_ a _proper_
	...
	* Brain 1.0
	...
	a)  Move your recommended editor to the config_auto.make file. Most people
	...
	b)  If the "setup" did not do the trick for you, you now really should
	   ...
	   inside the code and what failed and what's in config_auto.h (./configure
	   output), maybe a search through your system header can help to find the
	   missing magic knob.
	...
	inline asm, if not crashing all by itself.
	...
	i tested it didn't let me down. The last test with 2.x is loooong ago.
	...
	broken code for other archs and those guys with their new x86 (AVX)
	to trip over.</irony>
...
------------------
	...
	70's, or interstellar connections, but means these connections
...
$ dbus-send --system --dest=org.g2cd --print-reply /org/g2cd org.g2cd.version
...
==============
	...
	Other Bugs: - seems not 100% SMP safe...
...
--------------------------------------
	...
	and even starts up (and stays up).
	 ...
	 But it does _NOT_ work, and there are enough errors left.
	...
	- Get zlib and Berkley DB for mingw, they are available out there.
	 ...
	 Even with this "magic" shortcut, some stuff is simply _broken_, because
	...
	- G2CD uses pthreads. I added a small wrapper, but it is:
	  ...
	  It starts and so on, but it's not water tight and does hang on
	  shutdown.
	...
	- Which brings us to the major road block:
	  ...
	  they mix three things (to achive async io):
	   1) async transport to/from userland buffer with
	  ...
	  from the ground up with this, because:
	   a) our complete event control flow is wrong for this
	   b) because we do not churn much data.
...
===============
	...
	o) Even if C99 provides C++ like comments (//) and it is one of the
