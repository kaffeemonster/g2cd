/*
 * memneg.c
 * neg a memory region efficient, arm implementation
 *
...
#define SW_BODY(a_fix) \
	asm ( \
		...
		"vldmia.i64	%1!, {d1-d8}\n\t" \
		"pld	[%1, #64]\n\t" \
		"vext.8	d10, d0, d1, #"str_it(a_fix)"\n\t" \
		"vext.8	d11, d1, d2, #"str_it(a_fix)"\n\t" \
		"vext.8	d12, d2, d3, #"str_it(a_fix)"\n\t" \
		"vext.8	d13, d3, d4, #"str_it(a_fix)"\n\t" \
		"vext.8	d14, d4, d5, #"str_it(a_fix)"\n\t" \
		"vext.8	d15, d5, d6, #"str_it(a_fix)"\n\t" \
		"vext.8	d16, d6, d7, #"str_it(a_fix)"\n\t" \
		"vext.8	d17, d7, d8, #"str_it(a_fix)"\n\t" \
		"vmvn	q0, q5\n\t" \
		"vmvn	q1, q6\n\t" \
		"vmvn	q2, q7\n\t" \
		"vmvn	q3, q8\n\t" \
...
while(smaller_len--) \
	{ \
		v[0] = v[8]; /* swap from last round */ \
		v[1] = src_vec[1]; v[2] = src_vec[2]; v[3] = src_vec[3]; v[4] = src_vec[4]; /* load src */ \
		v[5] = src_vec[5]; v[6] = src_vec[6]; v[7] = src_vec[7]; v[8] = src_vec[8]; src_vec += 8; \
		vext_u8(v[0], v[1], a_fix); vext_u8(v[1], v[2], a_fix); vext_u8(v[2], v[3], a_fix); vext_u8(v[3], v[4], a_fix); /* permutate src for alignment */ \
		vext_u8(v[4], v[5], a_fix); vext_u8(v[5], v[6], a_fix); vext_u8(v[6], v[7], a_fix); vext_u8(v[7], v[8], a_fix); \
		v[0] = vmvn_u8(v[0]); v[1] = vmvn_u8(v[1]); v[2] = vmvn_u8(v[2]); v[3] = vmvn_u8(v[3]); /* neg it */ \
		v[4] = vmvn_u8(v[4]); v[5] = vmvn_u8(v[5]); v[6] = vmvn_u8(v[6]); v[7] = vmvn_u8(v[7]); \
		dst_vec[0] = v[0]; dst_vec[1] = v[1]; dst_vec[2] = v[2]; dst_vec[3] = v[3]; /* store it */ \
		dst_vec[4] = v[4]; dst_vec[5] = v[5]; dst_vec[6] = v[6]; dst_vec[7] = v[7]; dst_vec += 8; \
	} \
	for(; small_len--; dst_vec++, src_vec++) { \
		v[0] = v[8]; v[8] = src_vec[1]; \
		dst_vec[0] = vmvn_u8(vext_u8(v[0], v[8], a_fix)); \
	}
#endif
