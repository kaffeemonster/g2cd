...
alignment_16:
	if(len / SOVUC)
	{
		...
		f |= 512;
		vec_dst(src_vec1, f, 1);
		vec_dst(src_vec2, f, 2);
		...
		while(smaller_len)
		{
			...
			smaller_len -= f;
			for(; f--; dst_vec += 4, src_vec1 += 4, src_vec2 += 4)
			{
...
both_unaligned:
	if(len/(4*SOVUC))
	{
		...
		while(smaller_len)
		{
			...
			f = smaller_len >= 512 ? 512 : smaller_len;
			smaller_len -= f;
// TODO: reduce the number of vectors used?
			/* since we use the full reg set, we spill */
			while(f--)
			{
				/* load src2 */
				v[0] = v[8];
