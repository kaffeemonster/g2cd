...
alignment_16:
	if(len/16)
	{
		...
		/* get the vectorsave register, and set it to our used registers
		__asm__ __volatile__(
			"mfspr	%0,256\n\t"
			"mtspr	256,%1"
			: "=r" (old_vecsave)
			: "r" (0x03) // make sure this matches the bitmask of used register !!!
		);*/
		...
		while(small_len--)
		{
			*dst_vec = __builtin_altivec_vxor(*dst_vec, *src_vec);
/* my cross compiler bails out with a parsing error, fscking marcro-magic
		...
		/* restore the vectorsave register
		__asm__ __volatile__(
			"mtspr	256,%0"
			:
			: "r" (old_vecsave)
